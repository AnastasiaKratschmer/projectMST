...
def linear_align(gap, score_matrix, seq1, seq2, verbose=False):
    n = len(seq1)
    m = len(seq2)
...
def convert_distance_matrix_to_pseudomatrix(distance_matrix): #makes the distance matrix into the pseudomatrix format needed for selecting edges int the func. find_min_span
...
def find_min_span_edges_Kruskal(pseudomatrix, verbose=False): #the function actually implementing Kruskal's algo
    ...
    while len(set(name_dict.values()))>1: #so while all nodes are still not unified in one tree..
        ...
        if verbose: print("\n \n this is it "+str(it)+" of the find_min_span-func.") #the aforementioned print statement!
...
def equality_in_alignment_test(a,b):
    equality=False
...
def identify_merge_case(guide_PA,MSA1,MSA2,node1,node2,MSA_position_overview_dict,i,j,l):
    ...
    if equality_in_alignment_test(a,c)==True and equality_in_alignment_test(b,d)==True: case=3 #a==c and b==d
    elif equality_in_alignment_test(a,c)==False and equality_in_alignment_test(b,d)==False:
        if equality_in_alignment_test(b,c)==True:
            if guide_PA[i][0]!='-':
                k=1
        ...
        case=4
    elif equality_in_alignment_test(a,c)==True:#a==c
        if d!='-':
    ...
    elif equality_in_alignment_test(a,c)==False: #a!=c
        if a=='-':
...
def handle_merge_case1(MSA1,MSA2,i,j,l):
    how_many_cols_in_MSA1=len(MSA1[0])
    gaps_to_prepend=['-']*how_many_cols_in_MSA1
    new_col=gaps_to_prepend+list(MSA2[l])
    return new_col #after this, increase only l

def handle_merge_case2(MSA1,MSA2,i,j,l):
    how_many_cols_in_MSA2=len(MSA2[0])
...
def handle_merge_case3(MSA1,MSA2,i,j,l):
    col_new=list(MSA1[j])+list(MSA2[l])
    return col_new #after this, increase i, j and l

def handle_merge_case4(MSA1,MSA2,i,j,l):
    #col_new=list(MSA1[j])+list(MSA2[l])
    how_many_cols_in_MSA1=len(MSA1[0])
    how_many_cols_in_MSA2=len(MSA2[0])
    #make first column!
...
def merge_multiple(guide_PA,MSA_list,in_which_MSA_is_it,node1,node2):
    ...
    while i<=(len(guide_PA)-1) and j<=(len(MSA1)-1) and l<=(len(MSA2)-1):
        new_col=[]
        case,move_on_in_guide_flag,guide_was,MSA2_was,look_forward_flag1, look_forward_flag2=identify_merge_case(guide_PA,MSA1,MSA2,node1,node2, in_which_MSA_is_it,i,j,l)
    
        if case==1:
            col=handle_merge_case1(MSA1,MSA2,i,j,l)
            MSA_new.append(col) 
            ...
            new_col.append(col)
        if case==2:
            col=handle_merge_case2(MSA1,MSA2,i,j,l)
            MSA_new.append(col)
            if move_on_in_guide_flag==True:
                i+=1
            j+=1
            ...
            new_col.append(col)
        if case==3:
            col=handle_merge_case3(MSA1,MSA2,i,j,l)
            MSA_new.append(col)
            i+=1
            j+=1
            l+=1
            new_col.append(col)
        if case==4:
            col1,col2=handle_merge_case4(MSA1,MSA2,i,j,l)
...
def integrity_check_Kruskal_and_Prim(seqs, in_which_MSA_is_it, who_aligned_to_who, MSA_list,matrix, score_matrix, gap_cost):
    ...
    for element in who_aligned_to_who:
        ...
        if cost_after_MSA==matrix[int(seq1_nr)][int(seq2_nr)]:
            ...
            cost_for_suppesed_to_have_been=linear_align(gap_cost,score_matrix,seqs[int(seq1_nr)],seqs[int(seq2_nr)])
...
def integrity_check_Gusfield(seqs,M,score_matrix,gap_cost, matrix, s1_idx):
...
def extend_alignment_Gusfield(M, A):
    '''
...
def align_with_Gusfield(seqs, score_matrix, gap_cost, return_center_string=False, check_integrity=False):
    # STEP 1: Find the center string, s1
    matrix = np.full((len(seqs), len(seqs)), np.nan)
    # loop over all distinct pairs
    for i, seq1 in enumerate(seqs):
        for j, seq2 in enumerate(seqs):
            matrix[i, j] = get_cost_2(linear_align(gap_cost, score_matrix, seq1, seq2))
    ...
    cost_list = []
    for i in range(1, len(seqs)):
        cost = linear_align(gap_cost, score_matrix, s1, seqs[i])
        ...
        # extend
        Mk = extend_alignment_Gusfield(M, A)
        M = Mk

    # ACTUALLY COMPUTE (approximate) COST
    total_cost = compute_cost(M, score_matrix, gap_cost)
    if check_integrity == True:
        integrity_check_Gusfield(seqs, M, score_matrix,gap_cost,matrix,s1_idx)
    return matrix, M, total_cost,names, s1_idx

def perform_updates_Kruskal_and_Prim(in_which_MSA_is_it, node1, node2,united_MSA_new, MSA_list):
...
def align_with_Kruskal(seqs,score_matrix,gap_cost, check_integrity=False):
    ...
    matrix_for_MST=convert_distance_matrix_to_pseudomatrix(matrix_for_MST) #making the "pseudomatrix"
    #print("matrix for MST: "+str(matrix_for_MST))
    min_span_edges_res=find_min_span_edges_Kruskal(matrix_for_MST)
    in_which_MSA_is_it={}
    ...
    for row in min_span_edges_res:
        ...
        if row[0]=="*":
            ...
            cost=linear_align(gap_cost,score_matrix,seqs[int(node1)],seqs[int(node2)])
            alignment1_str,alignment2_str=linear_backtrack(seqs[int(node1)], seqs[int(node2)], cost, score_matrix, gap_cost)
            alignment1, alignment2 = [*alignment1_str], [*alignment2_str]
            A = [list(e) for e in zip(alignment1,alignment2)]
            united_MSA_new=merge_multiple(A,MSA_list,in_which_MSA_is_it,node1,node2)
            in_which_MSA_is_it, MSA_list=perform_updates_Kruskal_and_Prim(in_which_MSA_is_it, node1, node2, united_MSA_new, MSA_list)
    if check_integrity==True:
        integrity_check_Kruskal_and_Prim(seqs, in_which_MSA_is_it, who_aligned_to_who, MSA_list,matrix,score_matrix,gap_cost)
...
def make_str_from_cols(output_cols):
    #making each string into it's own list (instead of the lists being column) for outputting
    strings_list=[]
...
def align_with_Prim(seqs,score_matrix,gap_cost, check_integrity=False):
    # Make a matrix to hold pairwise alignment costs for all alignment combinations!
    matrix = np.full((len(seqs), len(seqs)), np.nan)
    # Loop over all pairs
    for i, seq1 in enumerate(seqs):
...
if __name__ == "__main__":
    ...
    print("Aligning the " + str(len(seqs)) + " sequences (guide tree type: "+str(type)+")...")
    print("Integrity check will be performed: "+ str(check))
    if type=="k":
        matrix, MSA_list, total_cost, in_which_MSA_is_it,who_aligned_to_who = align_with_Kruskal(seqs, score_matrix, gap,check_integrity=check)
        names=make_names_list(in_which_MSA_is_it)
        strings=make_str_from_cols(MSA_list[0])

    elif type=="p":
        matrix, MSA_list, total_cost, in_which_MSA_is_it, who_aligned_to_who = align_with_Prim(seqs, score_matrix, gap,check_integrity=check)
        names=make_names_list(in_which_MSA_is_it)
        strings=make_str_from_cols(MSA_list[0])
    elif type=="g":
    ...
    save_output = input("Do you want to save the output to a csv? (Y/n): ").strip().lower()

    if save_output == 'y':
        # Ask the user for a filename
        filename = input("Enter a filename (.csv will be added automatically): ")
