...
def alt_alt_merge_united(guide_PA,MSA_list,in_which_MSA_is_it,node1,node2):
    ...
    def integrity_check_OBO_and_gradual(seqs, in_which_MSA_is_it, who_aligned_to_who, MSA_list,matrix):
...
def new_assembly_OBO_x(seqs,score_matrix,gapcost, check_integrity=False):
...
def new_assembly_gradual_x(seqs,score_matrix,gapcost, check_integrity=False):
    # Make a matrix to hold pairwise alignment costs for all alignment combinations!
    matrix = np.full((len(seqs), len(seqs)), np.nan)
    # Loop over all pairs
    for i, seq1 in enumerate(seqs):
